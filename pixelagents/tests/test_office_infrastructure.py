...
from pixelagents.infrastructure.client_hub import ClientHub
from pixelagents.infrastructure.tickets import TICKET_TTL_SECONDS, TicketStore
from pixelagents.infrastructure.websocket import WebSocketServer
...
from pixelagents.tests.conftest import _FakeClientWebSocketResponse, _FakeWSMessage
...
class TestWebSocketServer(unittest.IsolatedAsyncioTestCase):
    async def test_bind_failure_cleans_partial_runner(self) -> None:
        ...
        with (
            patch("pixelagents.infrastructure.websocket.web.Application", return_value=app),
            patch("pixelagents.infrastructure.websocket.web.AppRunner", return_value=runner),
            patch("pixelagents.infrastructure.websocket.web.TCPSite", return_value=site),
        ):
    ...
    async def test_query_ticket_identity_is_available_to_application_handler(self) -> None:
        ...
        with patch(
            "pixelagents.infrastructure.websocket.web.WebSocketResponse", return_value=socket
        ):
