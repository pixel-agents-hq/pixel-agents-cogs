...
### `pixel-agents-consumer-contract.yaml`

`verify_outbound.py`'s check above validates captured messages directly
against the live vendor schema — useful, but every run needs the real
clone, and there's no reviewable record of "what we depend on" that
persists between runs. `pixel-agents-consumer-contract.yaml` adds that
record, generated by
[`contracts/pixel_agents/generate_consumer_contract.py`](../contracts/pixel_agents/generate_consumer_contract.py)
introspecting `pixelagents.contracts.outbound`'s `TypedDict`s (plain
`TypedDict`s, not pydantic — `model_json_schema()` doesn't apply here the
way it does for Pixel Index, so this walks `typing.get_type_hints`/
`__required_keys__` instead).

**Unlike Pixel Index's `contract.yaml`, this file is committed, not
gitignored.** CI regenerates it (`generate_consumer_contract.py --check`)
and fails on any diff from the committed copy, instead of silently
...
- **Live**, scheduled + PR-gated (a `consumer_contract_drift` check
  ...
  this contract declares? Walks every field the contract declares and
  confirms the live vendor schema still has it with a compatible shape
  (following one level of `$ref`, e.g. `agentStatus.status` →
  `AgentActivityStatus`), rather than only checking the fields our own
  builders happen to send on a given run.

This file is deliberately narrower than what
[`docs/corridor-pubsub-design.md`](corridor-pubsub-design.md) has verified
against upstream by hand (e.g. `agentDeselected`, `isHeadless`) — it only
covers what `outbound.py` actually builds today. See that doc's "Verifying
...
## corridor (design-only)

Not an external dependency like the two above — corridor's Pub/Sub event
bus (`docs/corridor-pubsub-design.md`) is an internal design that hasn't
been implemented yet.
[`contracts/corridor/corridor.yaml`](../contracts/corridor/corridor.yaml)
is hand-authored (there's no Python to introspect) from that doc's `AgentRef`
+ `AgentActivity` dataclasses.

Its CI check
([`contracts/corridor/lint_corridor_contract.py`](../contracts/corridor/lint_corridor_contract.py))
is **well-formedness only**: the yaml parses, its event set exactly matches
the design doc's six dataclasses, every name is cross-referenced against
the doc's own text, and every field has a declared type. It does not check
anything against real corridor code — `corridor/domain/models.py` doesn't
have any of these types yet. A real consumer-driven check (does
`corridor/domain/models.py` match `corridor.yaml`, the same way
`lint_outbound_contract.py` checks `pixelagents.contracts.outbound` against
`pixel-agents-consumer-contract.yaml` above) is explicit follow-up work,
listed in the design doc's implementation checklist, once corridor's
domain model actually exists to check against.
