...
the fake Discord-facing objects tests construct directly. A parallel copy of
painter/tests/conftest.py's shape, minus everything only painter's own
pixelagents/office-layout dependency needs (FakePixelAgents,
pixelagents_installable) -- animator has no such dependency."""
...
class FakeCorridor:
    ...
    *asks* corridor to reply/register with the right arguments -- what
    corridor actually renders/decides is covered by corridor's own test
    suite."""

    def __init__(self, *, mcp_tools: tuple[Any, ...] = ()) -> None:
...
class FakeCogManager:
    ...
    async def find_cog(self, name: str) -> FakeModuleSpec | None:
        ...
        installable = {"corridor": self.bot.corridor_installable}.get(name, True)
...
class FakeBot:
    ...
    unloaded, exercising CogBase.cog_load()'s auto-load-via-
    ensure_corridor_loaded path instead."""
...
class FakeEventQueue:
    ...
    `Message` protobuf objects, not further faked, since animator's own
...
class FakeRequestContext:
    ...
    `AnimatorAgentExecutor` actually reads."""
