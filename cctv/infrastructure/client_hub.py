...
Ported verbatim from floorplan's former `infrastructure/client_hub.py`
(now retired there, see docs/cctv-design.md) -- one instance per
pipeline (`CogBase` constructs two: `discord`, `editor`)."""
...
class ClientHub:
    ...
    async def send_to(self, socket: web.WebSocketResponse, message: Mapping[str, object]) -> bool:
        ...
        try:
            ...
            self._log.error(
                "cctv: refusing to send unserializable %s: %s", message.get("type"), exc
            )
            return False
        if socket.closed:
            self.remove(socket)
        ...
        try:
            ...
            self._log.debug("cctv: send error: %s", exc)
            self.remove(socket)
            return False
        return True
    ...
    async def broadcast(
        ...
        *,
        ...
        try:
            ...
            self._log.error(
                "cctv: refusing to broadcast unserializable %s: %s",
                message.get("type"),
                exc,
            )
            return 0
        ...
        for socket in list(self.clients):
            ...
            try:
                ...
                self._log.debug("cctv: broadcast error: %s", exc)
                self.remove(socket)
            else:
                sent += 1
        return sent
    ...
    async def close_all(self) -> None:
        ...
        for socket in list(self.clients):
            try:
                ...
                self._log.debug("cctv: client close error: %s", exc)
        self.clients.clear()
    ...
    async def reauthorize(self, authorize: Authorize) -> None:
        ...
        for state in list(self.clients.values()):
            ...
            try:
                ...
                self._log.error(
                    "cctv: could not reauthorize office user %d: %s",
                    state.user_id,
                    exc,
                )
