"""The Semantic Intermediate Representation (IR) for a Pixel Agents office
layout -- the shared model `architect` and `painter` both build their own
mutation services on top of. Originally `architect`'s own domain module
(`docs/architect-semantic-ir-design.md`); moved here so a second cog can
reach the same layout without depending on `architect` being loaded --
see `docs/painter-design.md` part A.

Zero framework imports -- no `pydantic`, no `redbot`, and no import of
this package's own `infrastructure/pixel_agents_adapter.py` or
`infrastructure/color_names.py` -- same "trivially unit-testable"
convention `architect/domain/models.py` used to follow, extended here to
"zero Pixel-Agents-*format*-specific imports" too: this module must never
know an asset ID string or HSB math itself. It *does* store raw tile
pattern integers (`TileCell.material`) -- a lossless-storage decision,
not a format-specific import, see docs/architect-semantic-ir-design.md
section 4.1 (v2).
"""
...
class TileCell:
    ...
    `color`/`raw_color` apply to WALL cells too, not just FLOOR --
    `tileColors[i]` carries a real, rendered per-tile HSB shift for walls
    in the upstream Pixel Agents webview (`wallTiles.ts`/`renderer.ts`'s
    `getColorizedWallSprite`/`wallColorToHex`, same shape/palette as
    floor tiles), architect's decoder previously just discarded it --
    see docs/painter-design.md part B. VOID cells never carry a color;
    ...
    color: str | None  # semantic color name (section 6.3), FLOOR or WALL
    ...
    def wall(
        ...
        *,
        ...
        return cls(
            TileKind.WALL, material=None, color=color, raw_color=raw_color, zone_label=zone_label
        )
