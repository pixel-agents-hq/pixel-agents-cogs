...
from pixelagents.contracts.websocket import (
    AuthorizeMessage,
    ClientMessage,
    ImportLayoutMessage,
...
class WebSocketServer:
    ...
    async def start(self, host: str, port: int) -> bool:
        ...
        try:
            ...
            try:
                ...
                self._log.debug("pixelagents: failed to clean up partial server: %s", cleanup_exc)
            self._log.error(
                "pixelagents: could not start office server on %s:%s — %s", host, port, exc
            )
            return False
        self.runner = runner
        self.site = site
        self._log.info("pixelagents: office server listening on %s:%s/ws", host, port)
        return True
    ...
    async def stop(self) -> None:
        ...
        if runner is not None:
            try:
                ...
                self._log.error("pixelagents: office server cleanup failed: %s", exc)
        ...
        try:
            async for incoming in socket:
                ...
                try:
                    ...
                    self._log.warning("pixelagents: invalid client JSON ignored: %s", exc)
                    continue
                await self.handle_payload(socket, payload)
        finally:
            self.clients.remove(socket)
            self._log.info(
                "pixelagents: office client disconnected (%d left)", self.clients.client_count
            )
        return socket
    ...
    async def handle_payload(self, socket: web.WebSocketResponse, payload: object) -> None:
        ...
        try:
            ...
            self._log.warning("pixelagents: invalid known client message ignored: %s", exc)
            return
        if message is None:
            return
    ...
    async def handle_message(self, socket: web.WebSocketResponse, message: ClientMessage) -> None:
        ...
        if isinstance(message, AuthorizeMessage):
            ...
            if is_editor:
                self._log.info("pixelagents: office client upgraded to editor")
            return
        ...
        if isinstance(message, _EDITOR_MESSAGE_TYPES) and not (
            ...
            self._log.info(
                "pixelagents: dropped %s from an unauthorized office client", message.type
            )
            return
        try:
            await self._handle_application_message(socket, message)
        except Exception as exc:
            self._log.error("pixelagents: client message error: %s", exc, exc_info=True)

    async def _authorize_safely(self, user_id: int) -> bool:
        try:
            return await self._authorize(user_id)
        except Exception as exc:
            self._log.error("pixelagents: authorization failed for user %d: %s", user_id, exc)
            return False
