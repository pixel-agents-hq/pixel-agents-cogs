...
class MemoryCatalogueRepository:
    """`set_layout` is no longer part of this Protocol at all -- layout
    storage moved to corridor, reached through pixelagents' facade, and
    `CatalogueService` now takes `set_layout` as a plain callable
    constructor argument instead (docs/cctv-design.md)."""
...
class TestCatalogueService(unittest.IsolatedAsyncioTestCase):
    def make_service(
        ...
        *,
        ...
        async def set_layout(layout: dict[str, object]) -> None:
            set_calls.append(layout)
    ...
    async def test_unauthorized_load_does_not_fetch_or_persist(self) -> None:
        service, repository, gateway, set_calls = self.make_service(authorized=False)
    ...
    async def test_invalid_layout_is_rejected_by_the_canonical_contract(self) -> None:
        service, repository, gateway, set_calls = self.make_service()
    ...
    async def test_valid_layout_is_preserved_and_persisted(self) -> None:
        service, repository, gateway, set_calls = self.make_service()
        raw_layout = {
            "version": 1,
            "cols": 1,
