...
`corridor.render_reply(guild_id, title=..., description=...)` instead: same
`ReplyMode` rendering, returned as a `RenderedReply` DTO instead of sent, so
the caller does its own send. pixelagents' `ReplyMixin`
