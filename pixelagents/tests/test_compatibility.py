"""Compatibility contracts that must survive the PixelAgents refactor."""
...
import pixelagents as _pixelagents_package
from pixelagents.pixelagents import pixelagents as PixelAgentsCog
from pixelagents.tests.conftest import _FakeClientWebSocketResponse, _FakeGroup
...
pixelagents_package = importlib.reload(_pixelagents_package)
pixelagents_module = importlib.import_module("pixelagents.pixelagents")
...
class TestPublicPackageContract(unittest.IsolatedAsyncioTestCase):
    async def test_setup_adds_the_exported_cog(self):
        ...
        await pixelagents_package.setup(bot)

        bot.add_cog.assert_awaited_once()
        assert isinstance(bot.add_cog.await_args.args[0], PixelAgentsCog)

    def test_lowercase_cog_export_remains_public(self):
        assert pixelagents_package.__all__ == ["pixelagents"]
        assert pixelagents_package.pixelagents is PixelAgentsCog
...
class TestConfigContract(unittest.IsolatedAsyncioTestCase):
    def test_identifier_scopes_and_defaults_are_stable(self):
        ...
        assert config.identifier == 0x706978656C61
...
class TestDashboardContract(unittest.IsolatedAsyncioTestCase):
    def test_page_metadata_and_required_context_parameters_are_stable(self):
        ...
        for method_name, expected in cases.items():
            method = getattr(PixelAgentsCog, method_name)
...
class TestServerContract(unittest.IsolatedAsyncioTestCase):
    async def test_server_registers_the_websocket_and_health_routes(self):
        ...
        with (
            patch.object(pixelagents_module.web, "Application", return_value=app),
            patch.object(pixelagents_module.web, "AppRunner", return_value=runner),
            patch.object(pixelagents_module.web, "TCPSite", return_value=site) as tcp_site,
        ):
...
class TestCommandContract:
    def test_command_tree_and_names_are_stable(self):
        ...
        assert root.command_metadata == {"name": "pixelagents", "invoke_without_command": True}
