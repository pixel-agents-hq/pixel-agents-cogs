...
builds its webview with `npm`/`vite`, and CCTV serves the result — a build-time
...
- [`contracts/pixel_agents/verify.py`](../contracts/pixel_agents/verify.py)
  ...
  runs the same checks a working office actually needs via CCTV's
...
Corridor's contracts are internal, committed artifacts generated from its real
domain models:

- `corridor/corridor.yaml` describes the agent activity event catalogue and is
  generated by `contracts.corridor.generate_corridor_contract` from
  `corridor.event_catalog.build_contract()`.
- `corridor/office_state.yaml` describes `OfficeState` and
  `OfficeStateChanged` and is generated by
  `contracts.corridor.generate_office_state_contract` from
  `corridor.office_state_catalog.build_contract()`.
