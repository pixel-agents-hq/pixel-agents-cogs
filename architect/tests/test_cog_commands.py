...
from .conftest import FakeBot, FakeContext, FakeLLMSettings, FakePixelAgents
...
class TestRefreshPixelagents(unittest.IsolatedAsyncioTestCase):
    ...
    architect's own reload left architect holding a stale `_pixelagents`
    Cog reference forever (its now-unloaded `_office_state` facade
    included), since `ensure_loaded` only resolves once, in architect's own
    `cog_load`. pixelagents now pushes its fresh instance to every loaded
    cog exposing `refresh_pixelagents` -- this is architect's side of it."""

    async def test_refresh_pixelagents_replaces_the_cached_reference(self) -> None:
        bot = FakeBot()
        cog = Architect(bot=bot)
        await cog.cog_load()
        fresh = FakePixelAgents(detail="fresh instance")
