"""ToolLoopService: painter's bounded tool-calling loop.

A deliberate parallel copy of `architect/application/tool_loop_service.py`
-- see docs/architect-design.md §8 on duplicating this shape per agent
rather than factoring it into a shared library ("revisit only once a
third agent needs the same shape" -- painter is that third agent, and the
decision made then, reaffirmed here, was to keep duplicating).

Painter has no other output channel at all -- its final plain-text
assistant reply *is* the result handed back over A2A. So this loop keeps
calling tools for as long as the model requests them, and stops as soon
as the model returns a turn with no tool calls, treating that turn's
`content` as the finished answer.
"""
...
class ToolLLM(Protocol):
    ...
    depends on -- always sends `tools`, mirrors architect's own ToolLLM."""
...
class ToolLoopService:
    ...
    async def run(
        ...
        *,
        ...
        than `on_activity`'s coarse summaries. See architect's own
        `tool_loop_service.py` for the full rationale, unchanged here."""
        ...
        while True:
            if calls_made >= max_tool_calls:
                log.warning("painter: tool loop hit max_tool_calls (%d), stopping", max_tool_calls)
            ...
            if not tool_calls:
                if debug:
                    log.info(
                        "painter: final answer with no tool calls this turn: %r",
            ...
            for call in tool_calls:
                if calls_made >= max_tool_calls:
                    log.warning(
                        "painter: tool loop hit max_tool_calls (%d), stopping", max_tool_calls
                    )
                    if debug and on_debug_event is not None:
                        await on_debug_event(f"stopping: hit max_tool_calls ({max_tool_calls})")
                    return ToolLoopResult(
...
async def _execute(
    ...
    convention (`status: Literal["ok", "error"]`) -- an `Output` with no
    `status` field at all is treated as successful, since nothing
    signaled a failure."""

    if debug:
        log.info("painter: tool call %s(%s)", call.function.name, call.function.arguments)
    tool = tools_by_name.get(call.function.name)
    if tool is None:
        if debug:
            log.info("painter: tool %s does not exist", call.function.name)
    ...
    try:
        ...
        if debug:
            log.info("painter: tool %s got invalid arguments: %s", call.function.name, exc)
    ...
    if debug:
        log.info("painter: tool %s returned %s", call.function.name, result_text)
