"""Unit tests for ensure_corridor_loaded (pixelagents/dependency_loader.py).
...
from pixelagents.dependency_loader import ensure_corridor_loaded
...
class TestEnsureCorridorLoaded(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_discovery_failure_reports_a_user_facing_load_error(self):
        bot = _bot(already_loaded=None)
        bot._cog_mgr.find_cog = AsyncMock(side_effect=RuntimeError("boom"))

        with self.assertRaisesRegex(CogLoadError, "discovery failed"):
            await ensure_corridor_loaded(bot)
