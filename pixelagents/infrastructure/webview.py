...
from pixelagents.contracts.layout import RawOfficeLayout
...
class WebviewAssetProvider:
    ...
    def load_assets(self) -> None:
        ...
        for name in ("characters", "floors", "walls", "carpets", "furniture"):
            ...
            try:
                ...
                self._log.error("pixelagents: could not read decoded %s: %s", name, exc)
        ...
        if catalog_path is not None:
            try:
                ...
                if isinstance(raw, list):
                    ...
                    self._log.error("pixelagents: furniture catalog is not a JSON array")
            except (OSError, UnicodeError, json.JSONDecodeError) as exc:
                self._log.error("pixelagents: could not read furniture catalog: %s", exc)

        # Keep the mapping identity stable for consumers holding the asset view.
        self.assets.clear()
        self.assets.update(loaded)
        self._log.info(
            "pixelagents: loaded assets — %d palettes, %d floors, %d wall sets, "
            "%d furniture sprites",
    ...
    def default_layout(self) -> RawOfficeLayout | None:
        ...
        try:
            ...
            self._log.warning("pixelagents: could not read bundled asset index: %s", exc)
            return None
        ...
        try:
            ...
            self._log.warning("pixelagents: could not read bundled default layout: %s", exc)
            return None
        if not isinstance(layout, dict):
            self._log.warning("pixelagents: bundled default layout is not a JSON object")
            return None
