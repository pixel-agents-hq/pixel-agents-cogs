...
| [`pixelagents`](../pixelagents) | Vendors and builds the Pixel Agents webview (clone + `npm`/`vite`) for other cogs to serve. No runtime/Discord surface of its own, but does own the Semantic IR domain model, Pixel Agents JSON codec, color palette, and the `OfficeStateFacade` — the one validated choke point every consumer (`cctv`, `floorplan`, `architect`, `painter`) reads/writes office layout/seats through, backed by corridor's own `OfficeState` aggregates (`docs/cctv-design.md`). | [pixelagents/README.md](../pixelagents/README.md) |
| [`floorplan`](../floorplan) | Browses the Pixel Index layout catalogue and loads a chosen layout into the shared Discord-page office layout, through pixelagents' `OfficeStateFacade`. No dashboard, WebSocket, or Discord-presence code of its own at all — `cctv` (below) owns that surface entirely; see `docs/cctv-design.md`. | [floorplan/README.md](../floorplan/README.md) |
| [`cctv`](../cctv) | The only dashboard-hosting cog: one aiohttp listener serving two independent Pixel Agents pages over Red Dashboard — a live Discord-presence view (`/third-party/cctv/discord`, ticket-gated editing) and an open structural/color editor view (`/third-party/cctv/editor`, no auth at all) — backed by corridor's `OfficeState` store via pixelagents' facade. Loading it is the only way either page (or any Discord-presence mirroring) exists; `floorplan`'s catalogue loads and `architect`'s/`painter`'s LLM-tool-driven layout mutations all keep working with `cctv` unloaded, since none of them depend on it. See `docs/cctv-design.md`. | [cctv/README.md](../cctv/README.md) |
...
| [`architect`](../architect) | A second, independent LLM agent -- never Discord-user-facing -- reachable only over the [A2A protocol](https://a2a-protocol.org/) on corridor's own shared listener (`docs/agent-directory-design.md`). Shares corridor's LLM connection with pico. Edits the shared "editor" office layout (a Semantic IR, `docs/architect-semantic-ir-design.md`) through pixelagents' `OfficeStateFacade`, via LLM tools and `[p]architect office ...` commands. Hosts no dashboard or WebSocket server of its own -- `cctv` is the only cog serving a browser-facing view of that layout. | [architect/README.md](../architect/README.md) |
