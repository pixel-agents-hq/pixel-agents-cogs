"""ToolLoopService: bootcamp's bounded tool-calling loop, shared by every
dynamically created custom agent.

Unlike pico's loop (which may only ever act via a tool call and never sends
raw LLM text anywhere), bootcamp has no other output channel at all -- its
final plain-text assistant reply *is* the result handed back, whether that
happens over A2A (a pico consultation) or directly (the `[p]bootcamp
<agent_key> <prompt>` command) -- see docs/bootcamp-design.md. So this loop
keeps calling tools for as long as the model requests them, and stops as
soon as the model returns a turn with no tool calls, treating that turn's
...
class ToolLoopService:
    ...
    async def run(
        ...
        *,
        ...
        streaming a live trace of this turn to an operator (e.g. bootcamp's
        ...
        while True:
            if calls_made >= max_tool_calls:
                log.warning("bootcamp: tool loop hit max_tool_calls (%d), stopping", max_tool_calls)
            ...
            if not tool_calls:
                if debug:
                    log.info(
                        "bootcamp: final answer with no tool calls this turn: %r",
            ...
            for call in tool_calls:
                if calls_made >= max_tool_calls:
                    log.warning(
                        "bootcamp: tool loop hit max_tool_calls (%d), stopping", max_tool_calls
                    )
                    if debug and on_debug_event is not None:
                        await on_debug_event(f"stopping: hit max_tool_calls ({max_tool_calls})")
                    return ToolLoopResult(
...
async def _execute(
    ...
    if debug:
        log.info("bootcamp: tool call %s(%s)", call.function.name, call.function.arguments)
    tool = tools_by_name.get(call.function.name)
    if tool is None:
        if debug:
            log.info("bootcamp: tool %s does not exist", call.function.name)
    ...
    try:
        ...
        if debug:
            log.info("bootcamp: tool %s got invalid arguments: %s", call.function.name, exc)
    ...
    if debug:
        log.info("bootcamp: tool %s returned %s", call.function.name, result_text)
