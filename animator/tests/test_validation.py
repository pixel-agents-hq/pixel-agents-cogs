"""parse_request_timeout/parse_read_timeout: pure parsing/validation, no
stubs needed."""
...
from ..adapters.validation import parse_read_timeout, parse_request_timeout
...
class TestParseReadTimeout(unittest.TestCase):
    def test_parses_a_positive_number(self) -> None:
        value, error = parse_read_timeout("180")

        self.assertEqual(value, 180.0)
        self.assertIsNone(error)

    def test_parses_a_float(self) -> None:
        value, error = parse_read_timeout("45.5")

        self.assertEqual(value, 45.5)
        self.assertIsNone(error)

    def test_default_literal_resets_to_none(self) -> None:
        value, error = parse_read_timeout("default")

        self.assertIsNone(value)
        self.assertIsNone(error)

    def test_none_literal_resets_to_none(self) -> None:
        value, error = parse_read_timeout("none")
    ...
    def test_rejects_a_non_numeric_string(self) -> None:
        value, error = parse_read_timeout("soon")

        self.assertIsNone(value)
        assert error is not None
        self.assertIn("not a valid read timeout", error)

    def test_rejects_zero(self) -> None:
        value, error = parse_read_timeout("0")
    ...
    def test_rejects_a_negative_number(self) -> None:
        value, error = parse_read_timeout("-5")
