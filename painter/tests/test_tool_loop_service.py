...
sequences. Unlike pico's loop (which never returns raw text), painter's
...
class StatusOutput(BaseModel):
    ...
    painter tool's `Output` follows (see office_tools.py) -- unlike
...
class TestToolLoopService(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_debug_off_emits_no_logs(self) -> None:
        ...
        with self.assertNoLogs("red.painter"):
            await service.run(
                base_url="https://x",
                api_key="k",
                model="m",
    ...
    async def test_debug_on_logs_the_tool_call_and_its_result(self) -> None:
        ...
        with self.assertLogs("red.painter", level="INFO") as captured:
            await service.run(
                base_url="https://x",
                api_key="k",
                model="m",
...
class TestOnActivity(unittest.IsolatedAsyncioTestCase):
    ...
    docs/corridor-pubsub-design.md's painter AgentReplied mapping."""
