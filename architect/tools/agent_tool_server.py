...
("architect")`) into architect's own `ToolSpec` Protocol.

Structurally identical to `pico/tools/cross_cog.py`'s `CrossCogTool`, but
implementing architect's *own* `ToolSpec` (`architect/tools/base.py`)
rather than pico's -- a deliberate duplication, matching that module's own
documented precedent that architect's `ToolSpec` is "a deliberate parallel
...
class AgentToolServerTool:
    """Wraps one corridor `RegisteredTool` as an architect `ToolSpec`."""
    ...
    def __init__(self, tool: RegisteredTool) -> None:
        ...
        # try/except building architect's tool list can log and skip just
