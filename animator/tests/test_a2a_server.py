...
own name/description/tag strings, and that `AnimatorAgentExecutor` wires
`agent_name="Animator"` through to `GenericAgentExecutor` correctly. The
...
from ..infrastructure.a2a_server import AnimatorAgentExecutor, build_agent_card
...
class TestAnimatorAgentExecutorWiring(unittest.IsolatedAsyncioTestCase):
    """A thin smoke test, not the full scenario matrix (see this module's
    own docstring): confirms `AnimatorAgentExecutor` actually reaches
    `GenericAgentExecutor`'s real behavior end to end, and that its
    error/log strings carry "Animator", not a generic placeholder."""
    ...
    async def test_names_painter_in_the_not_configured_message(self) -> None:
        ...
        self.assertIn("Animator", final_message.parts[0].text)
...
class TestBuildAgentCard(unittest.TestCase):
    ...
    def test_one_skill_per_tool(self) -> None:
        ...
        self.assertEqual([skill.id for skill in card.skills], ["deliver_pixel_agents_assets"])

    def test_skills_are_tagged_painter(self) -> None:
        card = build_agent_card(tools=[_StubTool()])

        self.assertEqual(list(card.skills[0].tags), ["animator"])

    def test_name_is_painter(self) -> None:
        card = build_agent_card(tools=[])

        self.assertEqual(card.name, "animator")
