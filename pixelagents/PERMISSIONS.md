...
pixelagents does not own its own permission model — it delegates every
...
describes what each corridor permission tier can do **from pixelagents'
point of view**. For how tiers are configured, see corridor's own docs /
`[p]corridorsettings`.

## Tiers and what they grant in pixelagents
...
| Building Manager | `building_manager` | Admin-assigned Discord role(s) | No — seeded by corridor as a default group, but pixelagents does not check this key anywhere today |
...
`pixelagents/adapters/office_gateway.py:_can_edit_layout_user` is the single
...
2. Otherwise, for each guild where pixelagents is enabled, pixelagents
...
Keyholder role assignment is entirely owned by corridor, not pixelagents:
...
pixelagents-specific permission command — changing corridor's `keyholder`
role set immediately changes who can edit the pixelagents office layout.
...
- Permission groups in corridor are **independent, unranked tiers** —
  ...
  versa. pixelagents only ever asks about `keyholder`.
- `building_manager` exists because corridor seeds it as one of two default
  groups for every guild; it is reserved for future use by pixelagents or
