"""Unit tests for contracts.corridor.lint_office_contract -- fully
...
from contracts.corridor import lint_office_contract as loc


class TestCheck(unittest.TestCase):
    def test_the_real_files_pass_with_zero_problems(self) -> None:
        contract = loc.load_contract()
        doc_text = loc.DESIGN_DOC_PATH.read_text(encoding="utf-8")

        problems = loc.check(contract, doc_text)
